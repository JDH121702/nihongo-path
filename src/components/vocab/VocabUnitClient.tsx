...
import { loadVocabPack, type LoadedVocabPack } from "@/data/vocab/load";
...
const BATCH = 15;
const EMPTY_WORDS: LoadedVocabPack["words"] = [];

type VocabLoadResult =
  | { requestKey: string; status: "ready"; data: LoadedVocabPack }
  | { requestKey: string; status: "not-found" | "error" };

export default function VocabUnitClient({ unitId }: { unitId: number }) {
  const { progress, introduceVocab, loaded, setScriptMode } = useProgress();

  const [batchIndex, setBatchIndex] = useState(0);
  const [flipped, setFlipped] = useState(false);
  const [sessionAdded, setSessionAdded] = useState(0);
  const [reload, setReload] = useState(0);
  const [loadResult, setLoadResult] = useState<VocabLoadResult | null>(null);
  const requestKey = `${unitId}:${reload}`;

  useEffect(() => {
    let cancelled = false;
    loadVocabPack(unitId)
      .then((data) => {
        if (cancelled) return;
        setLoadResult(
          data
            ? { requestKey, status: "ready", data }
      ...
      .catch(() => {
        if (cancelled) return;
        setLoadResult({ requestKey, status: "error" });
      });
  ...
  const currentResult =
    loadResult?.requestKey === requestKey ? loadResult : null;
  const vocabData = currentResult?.status === "ready" ? currentResult.data : null;
  const words = vocabData?.words ?? EMPTY_WORDS;
  ...
  if (currentResult.status === "error") {
    return (
      <div className="mx-auto max-w-lg px-4 py-20 text-center">
        <h1 className="text-xl font-bold">Couldn&apos;t load this vocab pack</h1>
        <p className="mt-2 text-sm text-muted">
          Your progress is safe. Check your connection and try loading the pack
          again.
        </p>
  ...
  if (currentResult.status !== "ready") return notFound();

  const { pack } = currentResult.data;
